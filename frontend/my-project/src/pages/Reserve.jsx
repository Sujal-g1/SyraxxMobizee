        ...
        <div className="flex justify-between items-end mb-8">
          ...
          >
            ...
            <p className="text-slate-500">
              {tab === "history"
                ? "Your Trips"
                : "Premium intercity travel at your fingertips."}
            </p>
          </motion.div>
              ...
              <div className="bg-slate-200/50 p-1.5 rounded-2xl flex gap-1">
                {[
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all ${tripType === t.id ? "bg-white shadow-sm text-black" : "text-slate-500 hover:text-slate-700"}`}
                  >
                    {t.icon} {t.label}
                  </button>
                ))}
              ...
              <div className="bg-white rounded-[2rem] p-6 shadow-xl shadow-slate-200/50 border border-slate-100">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <button
                      onClick={() => setForm({ ...form, ac: !form.ac })}
                      className={`w-full flex items-center justify-between px-5 py-4 rounded-2xl transition-all ${form.ac ? "bg-blue-50 text-blue-700 ring-1 ring-blue-200" : "bg-orange-50 text-orange-700 ring-1 ring-orange-200"}`}
                    >
                      <span className="font-semibold">
                        {form.ac ? "AC Premium" : "Non-AC Standard"}
                      </span>
                      <Wind
                        size={20}
                        className={form.ac ? "animate-pulse" : ""}
                      />
                    </button>
                  </div>
                </div>
                <div className="mt-6 flex gap-3">
                  {["shared", "private"].map((type) => (
                    <button
                      ...
                      className={`flex-1 py-3 rounded-xl capitalize font-bold text-sm transition-all ${form.bookingType === type ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}
                    >
                      {type}
                    </button>
                  ))}
                ...
                <motion.button
                  ...
                  disabled={loading}
                  className="w-full mt-8 py-4 bg-black hover:bg-gray-800 text-white rounded-2xl font-bold shadow-lg shadow-indigo-200 transition-all flex items-center justify-center gap-2"
                >
                  {loading ? (
              ...
              <div className="space-y-4">
                {results.map((opt, idx) => (
                  <motion.div
                    ...
                    className="group bg-white p-5 rounded-[1.5rem] flex items-center justify-between border border-slate-100 hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-500/5 transition-all"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 flex items-center gap-1">
                          <Users size={12} /> {opt.capacity} seats
                        </p>
                      </div>
                    </div>
                    <button
                      onClick={() => handleReserve(opt)}
                      className="bg-slate-900 text-white px-5 py-2 rounded-xl text-sm font-bold"
                    >
                      ₹{opt.totalPrice}
                    </button>
                  </motion.div>
              ...
              {history.map((item) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="font-black text-slate-900 uppercase text-xs tracking-tighter mb-1">
                      ID: {item._id.slice(-6)}
                    </p>
              ...
              <div className="p-8 space-y-6">
                <div className="flex justify-between items-end border-b border-dashed border-slate-200 pb-6">
                  <div>
                    ...
                    <p className="font-mono text-slate-600 text-sm uppercase">
                      {receipt?._id}
                    </p>
                  </div>
                  <div className="text-right">
              ...
              <div className="p-8 space-y-8">
                ...
                <div className="flex justify-between items-center py-6 border-y border-slate-50 border-dashed">
                  <div className="text-center">
                    ...
                    <p className="text-[10px] text-slate-400">CITY MAIN</p>
                  </div>
                  <div className="flex-1 px-4 text-indigo-200">
                    <div className="border-t-2 border-dashed border-indigo-100 w-full relative">
                      <Bus
                        size={16}
                        className="absolute -top-2 left-1/2 -translate-x-1/2 text-indigo-600 bg-white px-1"
                      />
                    </div>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-black text-slate-900">ARR</p>
                    <p className="text-[10px] text-slate-400">TERMINAL</p>
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-slate-800">AC-Tier</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-bold text-slate-400 uppercase">
                      Price
